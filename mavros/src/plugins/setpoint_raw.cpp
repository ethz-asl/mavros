...
class SetpointRawPlugin
 ...
 public:
  SetpointRawPlugin() : sp_nh("~setpoint_raw"), uas(nullptr){};
  ...
  void initialize(UAS &uas_) {
    ...
    local_sub =
        sp_nh.subscribe("local", 10, &SetpointRawPlugin::local_cb, this);
    global_sub =
        sp_nh.subscribe("global", 10, &SetpointRawPlugin::global_cb, this);
    attitude_sub =
        sp_nh.subscribe("attitude", 10, &SetpointRawPlugin::attitude_cb, this);
    rpyt_sub = sp_nh.subscribe("roll_pitch_yawrate_thrust", 10,
                               &SetpointRawPlugin::rpyt_cb, this);
    thrust_3d_sub = sp_nh.subscribe("thrust_3d", 10,
                                    &SetpointRawPlugin::thrust_3d_cb, this);
    target_local_pub =
        sp_nh.advertise<mavros_msgs::PositionTarget>("target_local", 10);
    target_global_pub =
        sp_nh.advertise<mavros_msgs::GlobalPositionTarget>("target_global", 10);
    target_attitude_pub =
        sp_nh.advertise<mavros_msgs::AttitudeTarget>("target_attitude", 10);
  }

  const message_map get_rx_handlers() {
    return {
        MESSAGE_HANDLER(MAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED,
                        &SetpointRawPlugin::handle_position_target_local_ned),
        MESSAGE_HANDLER(MAVLINK_MSG_ID_POSITION_TARGET_GLOBAL_INT,
                        &SetpointRawPlugin::handle_position_target_global_int),
        MESSAGE_HANDLER(MAVLINK_MSG_ID_ATTITUDE_TARGET,
                        &SetpointRawPlugin::handle_attitude_target),
    };
  }
 ...
 private:
  ...
  ros::Subscriber local_sub, global_sub, attitude_sub, rpyt_sub, thrust_3d_sub;
  ...
  /* -*- message handlers -*- */
  void handle_position_target_local_ned(const mavlink_message_t *msg,
    ...
    auto position =
        UAS::transform_frame_ned_enu(Eigen::Vector3d(tgt.x, tgt.y, tgt.z));
    auto velocity =
        UAS::transform_frame_ned_enu(Eigen::Vector3d(tgt.vx, tgt.vy, tgt.vz));
    auto af = UAS::transform_frame_ned_enu(
  ...
  void handle_position_target_global_int(const mavlink_message_t *msg,
                                         uint8_t sysid, uint8_t compid) {
  ...
  void handle_attitude_target(const mavlink_message_t *msg, uint8_t sysid,
                              uint8_t compid) {
    ...
    auto body_rate = UAS::transform_frame_baselink_aircraft(Eigen::Vector3d(
  ...
  void set_position_target_global_int(
      uint32_t time_boot_ms, uint8_t coordinate_frame, uint8_t type_mask,
      int32_t lat_int, int32_t lon_int, float alt, Eigen::Vector3d &velocity,
      Eigen::Vector3d &af, float yaw, float yaw_rate) {
    mavlink_message_t msg;
    mavlink_msg_set_position_target_global_int_pack_chan(
        UAS_PACK_CHAN(uas), &msg, time_boot_ms, UAS_PACK_TGT(uas),
        coordinate_frame, type_mask, lat_int, lon_int, alt, velocity.x(),
        velocity.y(), velocity.z(), af.x(), af.y(), af.z(), yaw, yaw_rate);
    UAS_FCU(uas)->send_message(&msg);
  }

  //! Message sepecification: @p
  //! http://mavlink.org/messages/common#SET_ATTITIDE_TARGET
  void set_attitude_target(uint32_t time_boot_ms, uint8_t type_mask,
    ...
    mavlink_msg_set_attitude_target_pack_chan(
        UAS_PACK_CHAN(uas), &msg, time_boot_ms, UAS_PACK_TGT(uas), type_mask, q,
        body_rate.x(), body_rate.y(), body_rate.z(), thrust);
    UAS_FCU(uas)->send_message(&msg);
  }
  ...
  void local_cb(const mavros_msgs::PositionTarget::ConstPtr &req) {
    ...
    set_position_target_local_ned(
        req->header.stamp.toNSec() / 1000000, req->coordinate_frame,
        req->type_mask, position.x(), position.y(), position.z(), velocity.x(),
        velocity.y(), velocity.z(), af.x(), af.y(), af.z(), yaw, yaw_rate);
  }
  ...
  void rpyt_cb(const mav_msgs::RollPitchYawrateThrustConstPtr msg) {
    ...
    // the masks are much more limited than the docs would suggest so we don't
    // use them
    uint8_t type_mask = 0;
    ...
    double thrust = std::min(
        1.0, std::max(0.0, msg->thrust.z * thrust_scaling_ * system_mass_kg_));
  ...
  void thrust_3d_cb(const geometry_msgs::PointStampedConstPtr msg) {
    mavlink_message_t mavlink_msg;

    mavlink_msg_debug_vect_pack_chan(
        UAS_FCU(uas)->get_system_id(), UAS_FCU(uas)->get_component_id(),
        UAS_FCU(uas)->get_channel(), &mavlink_msg, "thrust_3d",
        msg->header.stamp.toNSec() / 1000, msg->point.x, msg->point.y,
        msg->point.z);
    UAS_FCU(uas)->send_message(&mavlink_msg);
  }
};
};  // namespace mavplugin
